...
/**
 * @brief Класс ConsoleProgress показывает индикацию прогресса, на основе тектового представления путем вывода в указаный поток.
...
public:
    /**
     * @brief Конструктор
     * @param expectedCount Максимальное (ожидаемое) колчисево итераций
     * @param stream Ссылка на поток для вывода прогресса
     * @param s1 Строка, которая будет выведена перед цифрами идикации
    ...
    /**
     * @brief Увеличевает количество выполненых итераций на заданный шаг. Если необходимо, то выводит новый символ в строку прогресса.
...
private:
    ...
    unsigned long count_; //!< Пройденое количество итераций
