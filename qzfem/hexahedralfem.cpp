...
HexahedralFEM::HexahedralFEM(HexahedralMesh3D *mesh, const MechanicalParameters &parameters, FEMCondition3DPointer pressure, std::vector<FEMCondition3DPointer> boundary)
{
    const int freedom = 3;
    ...
    for (UInteger i = 0; i < elementsCount; i++)
    {
        ...
        if (mesh->isBorderElement(i))
        {
            ...
            for (int j = 0; j < element->facesCount(); j++)
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    if (mesh->nodeType(face[k]) == INNER || !pressure->isApplied(mesh->node(face[k])))
//                        if (mesh->nodeType(face[k]) == INNER || !( fabs(mesh->node(face[k])->y() - 0.018) < 0.0001 ))
                    {
                        isBorderFace = false;
                    }
                } // for k
                if (isBorderFace)
                {
                    ...
                    for (int k = 0; k < 4; k++)
                    {
                        force(face[k]) = force(face[k] ) + pressure->u() * faceArea / 4.0;
                        force(face[k] + nodesCount) = force(face[k] + nodesCount) + pressure->v() * faceArea / 4.0;
                        force(face[k] + 2L * nodesCount) = force(face[k] + 2L * nodesCount) + pressure->w() * faceArea / 4.0;
                    } // for k
                }
            } // for j
        } // if
