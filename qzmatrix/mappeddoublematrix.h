...
public:
     ...
     * @return Ссылка на вектор данных строки
     ...
     * Умножение "быстрое": перебираются только ненулевые элементы строки
     */
     ...
     * @param epsilon Точность метода
     * @param niter Максимальное количество итераций
     * @param printMessages Флаг печати сообщений
     * @param messageStep Шаг печати сообщений (через сколько итераций выводить сообщение)
     * @return Решение СЛАУ
     */
    ...
    /**
     * @brief Указатель на "гипотетический" последний элемент (адрес) строки матрицы (итератор)
     * @param i Номер строки
     * @return Константный итератор
     */
